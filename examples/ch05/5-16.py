...
async def generate_text(prompt: str, temperature: float = 0.7) -> str:
    ...
    try:
        ...
        return "Failed to obtain predictions from VVLM - See server logs for more details"
    try:
        output = predictions["choices"][0]["message"]["content"]
        logger.debug(f"Generated text: {output}")
        return output
    except KeyError as e:
        logger.error(f"Falied to parse predictions from VLLM - Error: {e}")
        return "Failed to pparse predictions from VLLM - See server logs for more details"