...
from database import engine, init_db, DBSessionDep
...
async def list_conversations_controller(session: DBSessionDep, skip: int = 0, take: int = 100 ) -> list[ConversationOut]:
    async with session.begin():
        result = await session.execute(select(Conversation).offset(skip).limit(take))
        return [
            ConversationOut.model_validate(conversation) for conversation in result.scalars().all()
        ]
...
@app.put("/conversation/{id}", status_code=status.HTTP_202_ACCEPTED)
